...
xhttp.onreadystatechange = function() {
    if (this.readyState === 4 && this.status === 200) {
        ...
        document.querySelector('#vault').innerHTML = userData.cookies;
        document.querySelector('#rank-text').innerHTML = userData.rank;
    }
};
...
function nextQuestion(listaretete) {
    ...
    if (stepSession === 5) {
        ...
        xxhttp.onreadystatechange = function () {
            if (this.readyState === 4 && this.status === 200) {
                let userData = JSON.parse(this.responseText);
                    let sessionScore = document.querySelector('#session-score-text').innerHTML;
                    fetch('users/' + id, {
                        method: 'put',
                        headers: {'Content-Type': 'application/json'},
                        body: JSON.stringify({
                            'totalScore': userData.totalScore + parseInt(sessionScore)
                        })
                    })
            };
        };
        xxhttp.open("GET", "users/" + id, true);
        xxhttp.send();
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            ...
            if (langLocalStorage[0] === "Romanian") {
                ...
                localStorage.setItem('language', JSON.stringify(lang));
                ro();
                console.log('english total rank');
...
document.querySelector('.home').addEventListener('click',function(){
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            let userData = JSON.parse(this.responseText);
            if (langLocalStorage[0] === "Romanian") {
                console.log('totalScore romanian');
                if (userData.totalScore <= 7) {
                    fetch('users/' + id, {
                        method: 'put',
                        headers: {'Content-Type': 'application/json'},
